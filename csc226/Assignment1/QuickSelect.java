...
public class QuickSelect {
    ...
    private static int quickSelect(int left, int right, int[] array, int k){
		...
		p = partition(left, right, array, p);
    	
    	//TODO recursively call quickSelect

		if(k<p){
			return quickSelect(left, p, array, k);
		}else if(k==p){
			return p;
		}else{
			return quickSelect(p, right, array, k);
		}
    ...
    private static int partition(int left, int right, int[] array, int pIndex){
		//find leftside that is greater than pivot
		//find rightside that is less than pivot
		//swap both, continue
		...
		while(1){
			while(array[curLeft]<pVal){
				curleft++;
			}
			while(array[curRight]>pVal){
				curRight--;
			}
			if(curLeft>pVal&&curRight<pVal){
				swap(array, curLeft, curRight);
				curLeft++;
				curRight--;
			}else if(curLeft==pVal && curRight==pVal){
				return curP;
			}else if(curRight<pVal){
				swap(array, curLeft, curP);
				swap(array, curLeft, curRight);
			}else{
				swap(array, curRight, curP);
				swap(array, curRight, curLeft);
			}
		}
